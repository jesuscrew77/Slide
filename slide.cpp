...
{
    slide_type = INVALID_TYPE;
    if(!star_slide_data_prepared) return SlideParameters();
...
{
    if(!star_slide_data_prepared) return TestSlideParameters();
    int count_of_stars = 0;//для подсчёта числа звезд, попавших на слайд
    ...
    for(int i = 0;i < filtered_l_st.size();i ++)
    {
        ...
        if((x_coord > 0) && (x_coord < slideData.slideSizeX)
                &&(y_coord > 0) &&(y_coord < slideData.slideSizeY))
        {
